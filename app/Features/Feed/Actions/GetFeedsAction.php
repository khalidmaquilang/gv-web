...
class GetFeedsAction
{
    public function handle(string $user_id, bool $only_followed = false): CursorPaginator
    {
        return Feed::query()
            ...
            ->selectRaw('exists(select * from `followers` where `followers`.`following_id` = `feeds`.`user_id` and `followers`.`follower_id` = ?) as is_author_followed_by_user', [$user_id])
            ->with(['user', 'content'])
            ->accessible($user_id)
            ...
            ->when($only_followed, function ($query) use ($user_id): void {
                $query->whereExists(function (Builder $subQuery) use ($user_id): void {
                    $subQuery->selectRaw(1)
                        ->from('followers')
                        ->whereColumn('followers.following_id', 'feeds.user_id')
