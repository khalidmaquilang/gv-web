...
{
    public function __invoke(VideoUploadData $request): void
    {
        $path = 'videos/'.Str::uuid().'.mp4';
        $path = $this->uploadFile($request->video->getRealPath(), $path);
        ...
        $video = Video::create([
            ...
            'video_path' => $path,
            'allow_comments' => $request->allow_comments ?? false,
            'privacy' => $request->privacy ?? VideoPrivacyEnum::PublicView,
